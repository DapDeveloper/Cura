...
*/
        Column
        {
            ...
            ToolbarButton2
            {
                ...
                visible: true
                toolItem: UM.RecolorImage2
                {
                    source: UM.Theme.getIcon("wizard") //!= "" ? UM.Theme.getIcon(model.icon) : "file:///" + model.location + "/" + model.icon
        ...
        {
            id: sliceButtons
            anchors.top: wizardButtons.bottom
            anchors.right: parent.right
            width: childrenRect.width
            height: childrenRect.height
            ToolbarButton2
            {
                id: sliceButton
                text: catalog.i18nc("@label", "Start the slicing process") 
                enabled: base.backendState != UM.Backend.Error
                visible: base.backendState == UM.Backend.NotStarted || base.backendState == UM.Backend.Error
             // visible:true
                toolItem: UM.RecolorImage2
                {
                    source: UM.Theme.getIcon("slicebutton") //!= "" ? UM.Theme.getIcon(model.icon) : "file:///" + model.location + "/" + model.icon
                    //color: UM.Theme.getColor("wizard")
                    //sourceSize: UM.Theme.getSize("button_icon")
                }
